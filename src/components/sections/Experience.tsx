...
function ExperienceModal({ item, isOpen, onClose }: { item: ExperienceItem; isOpen: boolean; onClose: () => void }) {
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={item.title} size="lg">
      <div className="space-y-5">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Badge label={item.type === 'work' ? 'Trabajo' : 'Educación'} variant="gradient" size="sm" />
        </div>
        ...
        <div className="flex flex-wrap gap-4 text-sm text-[#A0A0A0]">
          ...
          <span className="flex items-center gap-1.5">
            <Calendar size={14} />
            {formatDate(item.startDate)} — {item.current ? 'Actualidad' : item.endDate ? formatDate(item.endDate) : ''}
          </span>
        </div>

        <p className="text-[#A0A0A0] leading-relaxed whitespace-pre-line">{item.longDescription}</p>

        {item.achievements.length > 0 && (
          <div>
            <h3 className="text-sm font-semibold text-[#6B6B6B] uppercase tracking-wider mb-3">Logros</h3>
        ...
        {item.technologies.length > 0 && (
          <div>
            <h3 className="text-sm font-semibold text-[#6B6B6B] uppercase tracking-wider mb-3">Tecnologías</h3>
...
function ExperienceCard({ item }: { item: ExperienceItem }) {
  ...
  return (
    <>
      ...
      >
        <div className="flex items-start gap-4">
          <div className="shrink-0 w-10 h-10 rounded-xl gradient-bg flex items-center justify-center">
            {isWork ? <Briefcase size={18} className="text-white" /> : <GraduationCap size={18} className="text-white" />}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                <h3 className="font-semibold text-white text-sm group-hover:gradient-text transition-all">
                  {item.title}
                </h3>
            ...
            <p className="mt-3 text-xs text-[#A0A0A0] leading-relaxed line-clamp-2">{item.description}</p>
            <div className="mt-3 flex flex-wrap gap-1">
              {item.technologies.slice(0, 3).map((t) => (
                <Badge key={t} label={t} variant="default" size="sm" />
              ))}
              {item.technologies.length > 3 && (
                <Badge label={`+${item.technologies.length - 3}`} variant="outline" size="sm" />
              )}
            </div>
            <button
      ...
      <ExperienceModal item={item as ExperienceItem} isOpen={isOpen} onClose={close} />
    </>
  );
}
