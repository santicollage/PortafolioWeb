...
function AboutModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Sobre ${personalData.firstName}`} size="lg">
      <div className="space-y-5">
        ...
        <div className="text-center">
          <h3 className="text-xl font-bold text-white font-display">{personalData.name}</h3>
          <p className="text-sm gradient-text font-medium mt-1">{personalData.title}</p>
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          {highlights.map(({ icon: Icon, label, desc }) => (
            <div key={label} className="flex items-center gap-3 p-3 rounded-xl bg-[#2C2C2C]">
          ...
          <div className="lg:col-span-4">
            <div className="relative w-full max-w-xs mx-auto lg:mx-0">
              <div
                ...
                style={{ background: 'linear-gradient(135deg, #5919C2, #F68237)' }}
              />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {highlights.map(({ icon: Icon, label, desc }) => (
                ...
                >
                  ...
                  <div className="text-xs text-[#6B6B6B] leading-tight">{desc}</div>
                </div>
              ))}
            </div>

            <div className="p-6 rounded-2xl bg-[#1F1F1F] border border-[#2C2C2C]">
              <p className="text-[#A0A0A0] leading-relaxed">{personalData.bio}</p>
              <p className="mt-3 text-[#A0A0A0] leading-relaxed line-clamp-3">
                {personalData.bioExtended.split('\n\n')[1]}
              </p>
            </div>
